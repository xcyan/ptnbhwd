...
#define TH_GENERIC_FILE "generic/BilinearSamplerPerspective.c"
#else

#include <stdbool.h>
#include <stdio.h>
// code adapted from the github repo
...
static int nn_(BilinearSamplerPerspective_updateOutput)(lua_State *L)
{
  THTensor *inputImages = luaT_checkudata(L, 2, torch_Tensor);
  THTensor *grids = luaT_checkudata(L, 3, torch_Tensor);
  THTensor *output = luaT_checkudata(L, 4, torch_Tensor);
...
static int nn_(BilinearSamplerPerspective_updateGradInput)(lua_State *L)
{
  THTensor *inputImages = luaT_checkudata(L, 2, torch_Tensor);
  THTensor *grids = luaT_checkudata(L, 3, torch_Tensor);
  THTensor *gradInputImages = luaT_checkudata(L, 4, torch_Tensor);
...
static const struct luaL_Reg nn_(BilinearSamplerPerspective__) [] = {
  {"BilinearSamplerPerspective_updateOutput", nn_(BilinearSamplerPerspective_updateOutput)},
  {"BilinearSamplerPerspective_updateGradInput", nn_(BilinearSamplerPerspective_updateGradInput)},
...
static void nn_(BilinearSamplerPerspective_init)(lua_State *L)
{
  luaT_pushmetatable(L, torch_Tensor);
  luaT_registeratname(L, nn_(BilinearSamplerPerspective__), "nn");
  lua_pop(L,1);
}
